...
class CommentDaoImpl @Autowired constructor(
        ...
        val sql = """
            ...
            (
                ...
                FROM
                    comment
                WHERE
                    is_del = ${DBConst.falseVal}
                AND root_id = $id
            ) AS c2
            SET c1.sub_num = c2.num
            WHERE
                id = $id
            AND is_del = ${DBConst.falseVal}
        """.trimIndent()
    ...
    override fun delete(id: Long): Int {
        ...
        return this.namedParameterJdbcTemplate.update(sql + " WHERE is_del=${DBConst.falseVal} AND (id=$id OR path like '${comment.path + "_" + id}%')", paramMap)
    }
    ...
    override fun countRoot(type: Int, targetId: Long): Int {
        val sql = "SELECT COUNT(1) FROM comment WHERE type=$type AND is_del=${DBConst.falseVal} AND target_id=$targetId AND root_id=${DBConst.defaultParentId}"
    ...
    private fun listOfUserUnreadCommentDTO(type: Int, condition: String): MutableList<UserUnreadCommentDTO> {
        ...
        val sql = """
                SELECT
                    c.id,c.root_id,c.parent_id,c.path,c.type,target_id,target_uri,target_user_id,from_user_id,to_user_id,content,sub_num,c.create_datetime,c.is_del
                ...
                WHERE
                c.is_del = ${DBConst.falseVal}
                AND u1.is_del = ${DBConst.falseVal}
                AND u2.is_del = ${DBConst.falseVal}
                AND u3.is_del = ${DBConst.falseVal}
                AND t.is_del = ${DBConst.falseVal}
                AND c.id $condition
                ORDER BY id DESC
            """.trimIndent()
    ...
    override fun listOfTargetCommentDTO(type: Int, targetId: Long, rootId: Long, orderBy: String?, currPage: Int, pageSize: Int): MutableList<TargetCommentDTO> {
        ...
        val sql = """
            SELECT
                c.id,c.root_id,parent_id,c.path,c.type,target_id,target_uri,target_user_id,from_user_id,to_user_id,content,sub_num,c.create_datetime,c.is_del
            ...
            WHERE
            u1.is_del = ${DBConst.falseVal}
            AND u2.is_del = ${DBConst.falseVal}
            AND c.is_del = ${DBConst.falseVal}
            AND c.type = $type
